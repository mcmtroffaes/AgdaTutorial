...
\begin{code}
module Logic_a where
\end{code}

Zero order logic
================

Haskell's type system incorporates constructive zero-order logic (propositional logic).

Hilbert-style calculus of logic:
...
In constructive logic, axiom `exclMiddle` (the law of excluded middle, equivalent with `¬¬P ⇒ P`) is excluded, so indirect proofs are not allowed, but every proof constructs an algorithm. We show an example later.
`modusPonens` is provided as beta reduction.
...
The logic is not safe in Haskell because you can prove any statement with `undefined :: p` and you can define non-total functions.

Haskell exercises
=================

Prove that the previous axioms (except `exclMiddle`) hold in Haskell. The type is given for some of them as a help.

\begin{code}
{-# LANGUAGE RankNTypes, ImpredicativeTypes, LiberalTypeSynonyms #-}

type Not p  = (forall s. p -> s) -- existential type
type Bottom = Not ()
type Equiv p q = (p -> q, q -> p)
...
| conjIntr = (,)
-- disjIntr1    P ⇒ (P ∨ Q)
disjIntr1 ::    p -> Either p q
| disjIntr1 = Left
-- disjIntr2    Q ⇒ (P ∨ Q)
| disjIntr2 ::  q -> Either p q
| disjIntr2 = Right
-- disjElim     (P ⇒ R) ⇒ ((Q ⇒ R) ⇒ ((P ∨ Q) ⇒ R))
| disjElim ::   (p -> r) -> ((q -> r) -> (Either p q -> r))
| disjElim = either
-- reductioAbs  (P ⇒ Q) ⇒ ((P ⇒ ¬Q) ⇒ ¬P)
reductioAbs ::  (p -> q) -> ((p -> Not q) -> Not p)
| reductioAbs pq pnq p = (pnq p) (pq p)
-- contradict   P ⇒ (¬P ⇒ Q)
| contradict :: p -> (Not p -> q)
| contradict p np = np p
-- exclMiddle   P ∨ ¬P -- this cannot be proven without getting an exception or endless loop
exclMiddle ::   Either p (Not p)
| exclMiddle = exclMiddle
-- equivElim1   (P ⇔ Q) ⇒ (P ⇒ Q)
| equivElim1 :: Equiv p q -> (p -> q)
| equivElim1 = fst
-- equivElim2   (P ⇔ Q) ⇒ (Q ⇒ P)
| equivElim2 :: Equiv p q -> (q -> p)
| equivElim2 = snd
-- equivIntr    (P ⇒ Q) ⇒ ((Q ⇒ P) ⇒ (P ⇔ Q))
| equivIntr ::  (p -> q) -> ((q -> p) -> Equiv p q)
| equivIntr = (,)
...
Also prove that the following propositions hold:
...
| assoc (p, (q, r)) = ((p, q), r)
-- andIdemp    P ∧ P ⇔ P
| andIdemp ::    Equiv (p, p) p
| andIdemp = (fst, \p -> (p, p))
-- andOrDistr  P ∧ (Q ∨ R) ⇔ (P ∧ Q) ∨ (P ∧ R)
| andOrDistr ::  Equiv (p, Either q r) (Either (p, q) (p, r))
| andOrDistr = (toRight, toLeft) where
|   toRight (p, Left  q)  = Left  (p, q)
|   toRight (p, Right r)  = Right (p, r)
|   toLeft (Left  (p, q)) = (p, Left  q)
|   toLeft (Right (p, r)) = (p, Right r)
-- botUnit     ⊥ ∧ P ⇔ ⊥   -- hint: use lambda functions
| botUnit ::     Equiv (Bottom, p) Bottom
| botUnit = (\(bot, p) -> bot, \bot -> (bot, bot ()))
-- dupNeg      P ⇒ ¬¬P
| dupNeg ::      p -> Not (Not p)
| dupNeg p np = np p
-- contraPos   P ⇒ Q ⇒ (¬Q ⇒ ¬P)
...
-- deMorg1     ¬P ∧ ¬Q ⇒ ¬(P ∨ Q)
| deMorg1 ::     (Not p, Not q) -> Not (p, q)
| deMorg1 (np, nq) (p, q) = np p
-- deMorg2     ¬P ∨ ¬Q ⇒ ¬(P ∧ Q)
| deMorg2 ::     Either (Not p) (Not q) -> Not (p, q)
| deMorg2 (Left  np) (p, q) = np p
| deMorg2 (Right nq) (p, q) = nq q
\end{code}
